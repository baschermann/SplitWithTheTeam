function create_gui(player)
	...
	player.gui.center.container.add {
		type = "table",
		name = "table",
	...
	create_gui_list(
		player.gui.center.container.table,
		global.recipes,
		"Team 1",
		false,
		global.team_names[1],
...
function create_gui_list(element, recipes, caption, is_button, filter_name, player, column_count, width)
	...
	local inner =
		scrollpane.add {type = "table", name = "table" .. caption .. "4", direction = "vertical", column_count = column_count}
...
function get_other_force_name(name)
	if (global.team_names[1] == name) then
		return global.team_names[2]
	else
		return global.team_names[1]
	end
end
...
function assign_technologies(name, player)
	...
	for key, value in pairs(global.recipes) do
		if (value == "none") then
			free_technologies = free_technologies + 1
		end
	end

	--if(free_technologies <= 0) then assign_techs(player.force, other_force, global.start_techs, true, 0, player.force) end
	if (free_technologies <= 0) then
		assign_techs(global.start_techs, true, 0, player.force)
	end
end
...
function on_research_finished(event)
	...
	for key, value in pairs(event.research.effects) do
		if (value.type == "unlock-recipe") then
			if
				(game.recipe_prototypes[value.recipe].subgroup.name == "fill-barrel" or
					game.recipe_prototypes[value.recipe].subgroup.name == "empty-barrel")
			 then
...
function check_for_invalidity()
	for name, team in pairs(global.recipes) do
		if (team ~= "none" and team ~= "all") then
...
function on_built_entity(event)
	...
	elseif (entity.name == "offshore-pump" or entity.name == "pumpjack") then
...
function destroy_and_leave_on_ground(robot, entity)
	...
	entity.destroy()
	local new_entity =
		robot.surface.create_entity {name = "item-on-ground", position = robot.position, stack = {name = entity_name}}
